...
def plot_paid_vs_unpaid(df):
    ...
    plt.pie(paid_counts, labels=labels, autopct='%1.1f%%', startangle=90, colors=['lightgreen', 'lightcoral'])
...
def plot_stipend_distribution(df):
    plt.figure(figsize=(10, 5))
    sns.histplot(df['stipend_clean'], bins=10, kde=True, color='teal')
    plt.title('💰 Stipend Distribution')
    plt.xlabel('Stipend (USD)')
...
def plot_duration_distribution(df):
    plt.figure(figsize=(10, 5))
    sns.histplot(df['duration_days'], bins=10, kde=True, color='orange')
    plt.title('⏱️ Internship Duration Distribution')
