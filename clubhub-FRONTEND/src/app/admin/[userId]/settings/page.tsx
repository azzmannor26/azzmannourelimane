...
const Settings = ({ params }: { params: { userId: string } }) => {
  ...
  const userName = searchParams ? searchParams.get("name") ?? '' : ''; // Check if searchParams is defined before accessing
  ...
  return (
    <AdminLayout userId={userId} name={userName} profileImage={profileImage}> {/* Pass userId and name to AdminLayout */}
      <header className={styles.header}>
        <h2>Settings Page (for {userName || "User"})</h2> {/* Display user's name */}
        <div className={styles.adminInfo}>
          <span>{userName || "User"}</span> {/* Fallback if no name is available */}
          {/* Use the dynamic profile image */}
          ...
          <div className={styles.row}>
            <div>
              ...
              <input 
                ...
                value={userName || ""} // Display username in input field
            ...
            <div>
              ...
              <input 
                type="email" 
                placeholder={`${userName || "User"}@gmail.com`} // Default to name@gmail.com if no email is entered
                value={email}
