...
const Dashboard = ({ params }: { params: { userId: string } }) => {
  ...
  const name = searchParams ? searchParams.get("name") : null; // Ensure searchParams is not null before accessing
    ...
    <div className={styles.dashboard}>
      ...
      <div className={styles.mainContent}>
        <header className={styles.header}>
          <h2>Welcome {name || 'User'}!</h2> {/* Display user name, fallback to 'User' */}
          <div className={styles.adminInfo}>
            <span>{name || 'User'}</span> {/* Display user name in sidebar, fallback to 'User' */}
...
<div className={styles.uploadSection}>
  <input 
    ...
    id="file-upload" // Add an ID for reference if needed
  />
  {/* Remove the onClick and just allow file input */}
