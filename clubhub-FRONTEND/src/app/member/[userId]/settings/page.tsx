...
const name = searchParams?.get("name") || ""; // Use optional chaining to avoid error
  ...
  return (
    <AdminLayout userId={userId} name={name}> {/* Pass userId and name to AdminLayout */}
      <header className={styles.header}>
        <h2>Settings Page (for {name || "User"})</h2> {/* Display user's name */}
        <div className={styles.adminInfo}>
          <span>{name || "User"}</span> {/* Fallback if no name is available */}
