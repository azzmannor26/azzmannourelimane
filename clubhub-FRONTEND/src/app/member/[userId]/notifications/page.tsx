...
const MemberDashboard = () => {
  const params = useParams<{ userId?: string }>(); // Type params as potentially having userId
  const { userId } = params || {}; // Handle null or undefined case for params
  const searchParams = useSearchParams();
  const name = searchParams?.get("name") || "User"; // Fallback to "User" if name is not found

  const [notifications, setNotifications] = useState<any[]>([]); // Define the notifications state with a generic type
